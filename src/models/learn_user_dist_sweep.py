...
# todo: config this
sweep_selection = [8_000, 10_000, 12_000]
...
def main():
    ...
    for training_episode_id in range(config.config_learner.training_episodes):
        ...
        config.user_dist_average = config.rng.choice(sweep_selection)
        ...
        state_next = state_next / state_norm_factor  # TODO: smart?
    ...
    td3.networks['policy'][0]['primary'].save(
        Path(config.project_root_path, 'models', 'distance_sweep', config.config_learner.training_name, f'error_0.0'))
